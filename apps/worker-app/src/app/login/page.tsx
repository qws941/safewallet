...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiFetch<ApiResponse<AuthResponseDto>>('/auth/login', {
        method: 'POST',
        body: JSON.stringify({ 
          name: name.trim(),
      ...
      if (err instanceof Error) {
        ...
        setError('등록되지 않은 사용자입니다. 현장 관리자에게 문의하세요.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <Card className="w-full max-w-sm">
        ...
        <CardContent>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-2">
              <label htmlFor="dob" className="text-sm font-medium">생년월일 (6자리)</label>
              <Input
                id="dob"
                type="text"
                inputMode="numeric"
                ...
                value={dob}
                onChange={(e) => setDob(e.target.value.replace(/[^0-9]/g, '').slice(0, 6))}
                disabled={loading}
                maxLength={6}
              />
            </div>
